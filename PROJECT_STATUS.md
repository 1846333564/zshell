...
- 文件管理器在线文本编辑：双击或右键打开普通浮动窗口，支持多个文件窗口同时编辑、拖动、最小化、最大化、`Ctrl+S` 保存，关闭脏内容时提示保存、保存并关闭、不保存并关闭或取消；编辑区已重构为 Monaco Editor，支持代码高亮、Tab 缩进、`Ctrl+F` 搜索、匹配高亮和替换能力，打开文件时显示真实下载字节进度、速度和 Monaco 加载阶段进度。
