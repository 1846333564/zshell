...
- 连接标签只显示连接名。
- 文件管理器路径导航：固定根路径 `/`、解析后的 home 路径如 `/root`、树节点只显示 basename、目录树和右侧列表共享目录缓存与删除失效逻辑，目录树使用与右侧列表一致的分割线和选中条、路径输入支持全局已知目录的 Tab 唯一补全、路径历史按内存访问频次和最近访问排序并默认滚到底部，右侧居中折叠按钮、完整右键菜单动作、选中项删除确认、可调整文件列表列宽、列头点击排序动画和升降序箭头；已打开目录会先使用内存缓存秒开，再后台刷新真实目录内容，目录树索引按小批量写入，大目录预加载按低并发分批执行，手动刷新和 F5 会重新索引所有已打开目录。
- 文件管理器在线文本编辑：双击或右键打开普通浮动窗口，支持多个文件窗口同时编辑、拖动、最小化、最大化、`Ctrl+S` 保存，关闭脏内容时提示保存、保存并关闭、不保存并关闭或取消；编辑区已重构为 Monaco Editor，支持代码高亮、Tab 缩进、`Ctrl+F` 搜索、匹配高亮和替换能力，打开文件时在编辑器窗口内显示真实下载字节进度、速度和读取阶段，远程内容读完后会先用基础文本区可编辑显示，再在 Monaco 加载完成后无缝切换。
...
- Monaco Editor 通过动态 import 和 Web Worker 懒加载接入，应用首屏不静态加载 Monaco；启动后会延后到更空闲的阶段后台预热，首次打开在线编辑器会复用同一个加载 Promise，Monaco 未就绪时不会阻塞基础文本编辑。
...
- 在线编辑当前按文本处理并限制 64 MB；二进制安全编辑和显式编码选择是后续工作。
