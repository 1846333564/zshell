...
wiShell 是一个 Windows 桌面 SSH/SFTP 工具，技术栈包括 Go、Wails/WebView2、Vue、xterm.js、Monaco Editor 和基于 SSH 的 SFTP。当前版本从 `VERSION` 文件读取，本次版本为 `0.4.4`，版本号从 `0.0.1` 起步。发布产物输出到项目根目录的 `release` 文件夹，命名格式为 `wiShell.<版本号>.exe`；本地 `release` 历史包不会自动删除。
...
- `.github/workflows/release.yml` 用 GitHub Actions 在 tag 或手动触发时构建 Windows exe，对构建出的确切 EXE 执行启动和 `/api/health` 冒烟后创建或更新 GitHub Release 资产；`.github/release-names.json` 可为指定版本配置 Release 标题，本次 `0.4.4` 已配置“修复路径补全后焦点丢失”标题。
...
- 连接标签只显示连接名。
- 文件管理器路径导航：固定根路径 `/`、解析后的 home 路径如 `/root`，左侧目录树只显示目录且每个目录独占一行，不再压缩唯一连续子目录；左侧使用与 VS Code 同类的 90 度空心折线图标，不再把展开功能放在独立按钮上，整行点击会选中并在展开/折叠间切换，刷新目录时保留用户的折叠状态。目录树和右侧列表使用独立且明确的右键目标规则，右侧空白/表头回退到当前父目录，目录与文件拥有分离菜单，支持刷新、展开、整目录下载、原位重命名、上传、复制、红色剪切/删除、条件粘贴和复制路径；菜单不再显示顶部路径标题，并在渲染后按真实高度适配视口，极矮窗口使用菜单内滚动兜底。目录树与文件列表共享目录缓存及动作后的定向刷新，右侧点击会把左树定位到对应父目录；文件和目录列表使用固定行高虚拟滚动，在千级项目下只渲染可见窗口。路径输入复用递归预加载缓存中的父目录级有序索引，实际输入且有匹配时显示最多 10 个直接子目录候选，Tab 按唯一目录、公共前缀、精确目录与唯一更长候选规则补全，并同步导航左树和右侧列表；Enter 优先确认精确目录；补全导航期间会忽略输入框因加载禁用而触发的重复 `change`，防止旧草稿路径覆盖正确候选，并在补全导航结束后恢复地址栏焦点和末尾光标，支持连续输入下一级路径。路径历史按内存访问频次和最近访问排序并默认滚到底部，文件列表支持可调列宽、排序动画，并复用同一个空心折线图标表示升降序；已打开目录会先使用内存缓存秒开，再后台刷新真实目录内容，目录预加载在当前目录加载完成后立即后台启动，每批最多缓存 10 个未加载目录，并会基于已缓存目录继续递归推进，手动刷新和 F5 会重新索引所有已打开目录。
