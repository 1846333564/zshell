...
zShell 是一个 Windows 桌面 SSH/SFTP 工具，技术栈包括 Go、Wails/WebView2、Vue、xterm.js 和基于 SSH 的 SFTP。当前版本从 `VERSION` 文件读取，本次版本为 `0.1.7`，版本号从 `0.0.1` 起步。发布产物输出到项目根目录的 `release` 文件夹，命名格式为 `zshell.<版本号>.exe`；本地 `release` 历史包不会自动删除。
...
- `.github/workflows/release.yml` 用 GitHub Actions 在 tag 或手动触发时构建 Windows exe，并创建或更新 GitHub Release 资产；`.github/release-names.json` 可为指定版本配置 Release 标题，本次 `0.1.7` 标题为“优化文件管理器交互和上传进度”。
...
- 连接标签只显示连接名。
- 文件管理器路径导航：固定根路径 `/`、解析后的 home 路径如 `/root`、树节点只显示 basename、目录树使用与右侧列表一致的分割线和选中条、路径输入支持已知目录的 Tab 唯一补全、路径历史可弹出查看，右侧居中折叠按钮、完整右键菜单动作、选中项删除确认和可调整文件列表列宽；已打开目录会先使用内存缓存秒开，再后台刷新真实目录内容。
