...
zShell 是一个 Windows 桌面 SSH/SFTP 工具，技术栈包括 Go、Wails/WebView2、Vue、xterm.js、Monaco Editor 和基于 SSH 的 SFTP。当前版本从 `VERSION` 文件读取，本次版本为 `0.3.15`，版本号从 `0.0.1` 起步。发布产物输出到项目根目录的 `release` 文件夹，命名格式为 `zshell.<版本号>.exe`；本地 `release` 历史包不会自动删除。
...
- `.github/workflows/release.yml` 用 GitHub Actions 在 tag 或手动触发时构建 Windows exe，并创建或更新 GitHub Release 资产；`.github/release-names.json` 可为指定版本配置 Release 标题，本次 `0.3.15` 已配置“优化在线编辑首屏加载速度”标题。
...
- Monaco Editor 通过动态 import 和 Web Worker 懒加载接入，应用首屏不静态加载 Monaco；启动后会延后到更空闲的阶段后台预热，在线编辑器窗口打开后即可挂载 Monaco，远程内容按 32 KiB 分块追加到已有编辑器模型，首次打开会复用同一个加载 Promise。
