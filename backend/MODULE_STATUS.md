...
后端会先初始化日志系统，再在动态高端口启动本地 API/WebSocket 服务，并通过 Wails 加载前端资源。本地 API 使用请求头读取超时和空闲超时，但不为长时间 SFTP 上传、下载、更新等流式响应设置 30 秒写超时。日志写入 `%AppData%\zShell\log`，按小时轮转并在启动时清理 24 小时以前的日志。Wails 使用无边框 Windows 窗口，WebView2 GPU 已禁用以降低启动高负载阶段渲染进程崩溃风险，同时保留 WebView 右键事件，使 Vue 可以渲染自定义文件管理器菜单并屏蔽非文件区菜单。保存的连接配置在 API 服务启动时加载进内存，供终端、SFTP 和监控流程使用。
