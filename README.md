...
- 当前本地版本：`0.4.3`
- 版本来源：根目录 `VERSION`
- 本地打包产物：`release/wiShell.0.4.3.exe`
- 本版本会通过 `v0.4.3` tag 发布 GitHub Release，标题为“修复路径补全重复导航”。
...
- 文件路径输入会复用递归预加载缓存中的父目录索引，仅在实际输入且存在匹配时显示最多 10 个直接子目录候选；Tab 支持唯一目录、公共前缀、精确目录与唯一更长候选的多级联动补全，并同步导航文件树和文件列表，Enter 可优先确认精确目录；补全导航期间会忽略输入框因加载禁用而触发的重复 `change`，避免正确候选被旧草稿路径覆盖。
