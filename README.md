...
- 当前本地版本：`0.4.4`
- 版本来源：根目录 `VERSION`
- 本地打包产物：`release/wiShell.0.4.4.exe`
- 本版本会通过 `v0.4.4` tag 发布 GitHub Release，标题为“修复路径补全后焦点丢失”。
...
- 文件路径输入会复用递归预加载缓存中的父目录索引，仅在实际输入且存在匹配时显示最多 10 个直接子目录候选；Tab 支持唯一目录、公共前缀、精确目录与唯一更长候选的多级联动补全，并同步导航文件树和文件列表，Enter 可优先确认精确目录；补全导航期间会忽略输入框因加载禁用而触发的重复 `change`，导航完成后恢复地址栏焦点并把光标放到路径末尾，便于继续输入下一级目录。
